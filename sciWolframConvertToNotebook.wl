...
sciWolframExpr[expr_] := Module[{exprNB},
    If[MatchQ[First[expr], TextCell[__]],
        ...
        exprNB
        ,
        exprNB = expr /. basicReplace /. systemSymbolsReplace; ExpressionCell[exprNB, "Input"]
    ]
];

sciWolframConvert[file_] := Module[{fileBaseName, dir, filePDF, fileNB, exprs, cells, notebook},
    ...
    UsingFrontEnd[
        ...
        If[StringQ @ Environment["WSL_DISTRO_NAME"],
            ...
            ,
            SystemOpen[filePDF]; SystemOpen[fileNB]
        ];
    ];
